...
# based on one or more key fields
# assumes the first line of the CSV file is the header/defintion of the CSV
def readcsv2dict_with_header(csvfile, dictkeys, dupkeyfail=False, noshowwarning=False, headerlc=False,
                             encoding='windows-1252', debug=False):
    results = {}
    dupkeys = []
    dupcount = 0
    with open(csvfile, mode='r', encoding=encoding) as csv_file:
...
# based on one or more key fields
# assumes the first line of the CSV file is the header/defintion of the CSV
def readcsv2dict(csvfile, dictkeys, dupkeyfail=False, noshowwarning=False, headerlc=False, encoding='windows-1252',
                 debug=False):
    results, header, dupcnt = readcsv2dict_with_header(csvfile, dictkeys, dupkeyfail=dupkeyfail,
                                                       noshowwarning=noshowwarning, headerlc=headerlc,
...
def readcsv2dict_with_noheader(csvfile, dictkeys, header, dupkeyfail=False, noshowwarning=False,
                               encoding='windows-1252', debug=False):
    if not dictkeys:
...
def readcsv2dict_findheader(csvfile, req_cols, dictkeys, xlatdict={}, optiondict={}, col_aref=None, debug=False,
                            dupkeyfail=False):
    # check inputs
    if not dictkeys:
    ...
    results = readcsv2list_findheader(csvfile, req_cols, xlatdict=xlatdict, optiondict=optiondict, col_aref=col_aref,
                                      debug=debug)
