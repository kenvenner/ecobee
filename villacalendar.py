...
def read_future_calendar_events(service, now, debug=False):
    ...
    if 0:
        ...
        if debug:
            print(
                "read_future_calendar_events:Getting the upcoming 10 events:utcnow:",
                utcnow,
            )
        events_result = (
            service.events()
        ...
        logger.info(
            "called service.events - sleep for 2 seconds to avoid rate limiting"
        )
        time.sleep(2)
        ...
        if debug:
            print(
                "read_future_calendar_events:Getting all upcoming events:utcnow:",
                utcnow,
            )
        events = list()
        loopcnt = 0
        cal_request = service.events().list(
            calendarId="primary",
            timeMin=utcnow,
            maxResults=10,
            singleEvents=True,
        ...
        while cal_request is not None:
            ...
            if debug:
                print(
                    "read_future_calendar_events:event after run:",
                    loopcnt,
                    ":event_count:",
                    len(events),
                )
...
def event_already_exists(starttime, event, calexist_dict, debug=False):
    ...
    if gstarttime in calexist_dict:
        if debug:
            if debug:
                print(
                    "event_already_exists:Cal entry time/date match:", event["summary"]
                )
        if calexist_dict[gstarttime]["summary"] == event["summary"]:
            calexist_dict[gstarttime]["eventmatch"] = True
            logger.info(
                "Calendar entry exists:marked and skipping:%s", event["summary"]
            )
            # debugging
            if debug:
                print(
                    "event_already_exists:Cal entry exists - marked and skipping:",
                    event["summary"],
                )
            return True
...
def create_cal_event_start(service, calexist_dict, caldate, stay, debug=False):
    ...
    endtime = datetime.datetime.combine(
        caldate.date(), datetime.time(hour=23, minute=59)
    )

    # build the event dictionary
    eventbody = create_event_dict(
        who_by_type(stay["Type"]) + "-start", starttime, endtime, stay, debug=False
    )
    ...
    # logging
    logger.info(
        "Calendar event created:start:%s:%s", event.get("id"), eventbody["summary"]
    )
...
def create_cal_event_stay(service, calexist_dict, caldate, stay, debug=False):
    ...
    # build the event dictionary
    eventbody = create_event_dict(
        who_by_type(stay["Type"]) + "-stay", starttime, endtime, stay, debug=False
    )
    ...
    # logging
    logger.info(
        "Calendar event created:stay:%s:%s", event.get("id"), eventbody["summary"]
    )
...
def create_cal_event_exit(service, calexist_dict, caldate, stay, debug=False):
    ...
    # goes to just before noon
    endtime = datetime.datetime.combine(
        caldate.date(), datetime.time(hour=12, minute=0)
    )

    # build the event dictionary
    eventbody = create_event_dict(
        who_by_type(stay["Type"]) + "-exit", starttime, endtime, stay, debug=False
    )
    ...
    # logging
    logger.info(
        "Calendar event created:exit:%s:%s", event.get("id"), eventbody["summary"]
    )
...
def create_cal_event_start_exit(service, startdate, enddate, stay, debug=False):
    ...
    # logging
    logger.info(
        "Calendar event created:start_exit:%s:%s", event.get("id"), eventbody["summary"]
    )
...
def cal_events_dict_on_startdatetime(cal_events, service, delDupes=False, debug=False):
    ...
    for cal_event in cal_events:
        if cal_event["start"]["dateTime"] in cal_events_start_dict:
            ...
            if debug:
                print(
                    "cal_events_dict_on_startdatetime:duplicate calendar entry on start datetime:",
                    cal_event["start"]["dateTime"],
                )
            # and if we flagged it - we delete the current record and keep the prior
            if delDupes:
                logger.info(
                    "deleting the first duplicate record now id:%s:%s",
                    cal_events_start_dict[cal_event["start"]["dateTime"]]["id"],
                    cal_events_start_dict[cal_event["start"]["dateTime"]]["summary"],
                )
                # debugging
                if debug:
                    print(
                        "cal_events_dict_on_startdatetime:deleting the first duplicate record now id:",
                        cal_events_start_dict[cal_event["start"]["dateTime"]]["id"],
                    )
                # delete this event
                delete_cal_event(
                    service, cal_events_start_dict[cal_event["start"]["dateTime"]]["id"]
                )
...
def create_cal_events_for_villa_stays(
    service, xlsaref, cal_events_start_dict, ignorebefore, debug=False
):
    ...
    for stay in xlsaref:
        ...
        if stay["startdate"] < ignorebefore:
            ...
            if debug:
                print(
                    "create_cal_events_for_villa_stays:Stay already started get next stay:",
                    stay["startdate"],
                )
            continue
        ...
        id = create_cal_event_start(
            service, cal_events_start_dict, caldate, stay, debug=debug
        )
        ...
        while caldate < stay["exitdate"]:
            ...
            if debug:
                print(
                    "create_cal_events_for_villa_stays:in loop\ncreate_cal_events_for_villa_stays:caldate:",
                    caldate,
                )
            # create the stay event
            id = create_cal_event_stay(
                service, cal_events_start_dict, caldate, stay, debug=debug
            )
            # increment the date
            caldate += addoneday
...
def remove_nonmatch_events(service, cal_events_start_dict, debug=False):
    for caldatetime in cal_events_start_dict:
        if "eventmatch" not in cal_events_start_dict[caldatetime]:
            logger.info(
                "No match on event-removing event:%s:%s",
                cal_events_start_dict[caldatetime]["id"],
                cal_events_start_dict[caldatetime]["summary"],
            )
            # this did not have a match - show the ID (in the future just delete this entry
            if debug:
                print(
                    "remove_nonmatch_events:no match on event:",
                    caldatetime,
                    ":id:",
                    cal_events_start_dict[caldatetime]["id"],
                )
            # now just delete it
            delete_cal_event(
                service, cal_events_start_dict[caldatetime]["id"], debug=debug
            )
...
def sync_villa_cal_with_bp_xls(xlsaref, now=None, debug=False):
    ...
    cal_events_start_dict = cal_events_dict_on_startdatetime(
        cal_events, service, delDupes=True, debug=debug
    )
    ...
    create_cal_events_for_villa_stays(
        service, xlsaref, cal_events_start_dict, now, debug=debug
    )
...
if __name__ == "__main__":
    logging.basicConfig(
        filename=os.path.splitext(kvutil.scriptinfo()["name"])[0] + ".log",
        level=logging.INFO,
